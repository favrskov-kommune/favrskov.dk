...
curl -s -X POST -H 'Content-type: application/json' --data "{\"attachments\": [{\"author_icon\": \"https://staging.favrskov.drupal.dk/favrskov-logo.png\", \"author_name\": \"Favrskov.dk\", \"author_link\" : \"$7\", \"text\": \":azure-logo: Release of branch \`$2\` to <$7|$8> started...\",\"color\": \"E6E6E6\",\"mrkdwn_in\": [\"title\",\"text\"]}]}" $3

# If master branch also notify in #notif-deployments
if [ $2 = "master" ]; then
    curl -s -X POST -H 'Content-type: application/json' --data "{\"attachments\": [{\"author_icon\": \"https://staging.favrskov.drupal.dk/favrskov-logo.png\", \"author_name\": \"Favrskov.dk\", \"author_link\" : \"$7\", \"text\": \":azure-logo: Release of branch \`$2\` to <$7|$8> started...\",\"color\": \"E6E6E6\",\"mrkdwn_in\": [\"title\",\"text\"]}]}" $4
fi
