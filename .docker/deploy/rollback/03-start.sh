...
curl -s -X POST -H 'Content-type: application/json' --data "{\"attachments\": [{\"title\": \":favrskov-logo: Favrskov.dk\", \"title_link\": \"$7\", \"text\": \":white_check_mark: Release of branch \`$2\` to <$7|$8> successfully rolled back.\",\"color\": \"4AB441\",\"mrkdwn_in\": [\"title\",\"text\"]}]}" $3

# If master branch also notify in #notif-deployments
if [ $2 = "master" ]; then
    curl -s -X POST -H 'Content-type: application/json' --data "{\"attachments\": [{\"title\": \":favrskov-logo: Favrskov.dk\", \"title_link\": \"$7\", \"text\": \":white_check_mark: Release of branch \`$2\` to <$7|$8> successfully rolled back.\",\"color\": \"4AB441\",\"mrkdwn_in\": [\"title\",\"text\"]}]}" $4
fi