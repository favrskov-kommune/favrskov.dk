...
apt-get -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install sudo >/dev/null
apt-get -y --force-yes install python 2.7.9
apt-get -y --force-yes install python-dev python-simplejson curl make rsync git libmysqlclient-dev apparmor-utils >/dev/null
